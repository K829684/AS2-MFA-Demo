    ...
    if (userEmail && credentials) {  
      ...
      try {  
        ...
        if (response.ok) {  
          ...
          const textURL = new TextEncoder().encode(data.streamingUrl);

          const encodedURL = Buffer.from(textURL).toString('base64');
          
          // 將 URL 的開頭改為 amazonappstream:  
          // const appStreamingUrl = data.streamingUrl.replace(/^http/, 'amazonappstream:');  
          const appStreamingUrl = `amazonappstream:${encodedURL}`
